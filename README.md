...
│           │ Snowflake (SiS) │   │ (Snowflake           │ │
│           │ ダッシュボード    │   │  Intelligence)       │ │
│           └─────────────────┘   └─────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```
...
| **TV_VIEWING_LOG** | 5,000 | テレビ実視聴ログ。CM接触有無、チャンネル、番組、視聴秒数 |
...
ダッシュボードは7つのセクションで構成され、STADIA360のデータを多角的に可視化します。

### ダッシュボードセクション

| セクション | 主な可視化 |
|---|---|
| **KPI サマリー** | CM接触率、サイトCVR、平均購買額、平均NPS の4大指標 |
| **テレビ視聴分析** | チャンネル別CM接触率、時間帯別視聴パターン、デバイス別分布 |
| **態度変容ファネル** | CM接触者 vs 非接触者のリフト比較（認知→興味→検討→購入） |
| **サイト来訪分析** | 流入経路別セッション数・CVR、デバイス別分布 |
| **オフライン購買分析** | CM接触者/非接触者の購買金額比較、商品カテゴリ別売上 |
| **来店・アプリ分析** | 来店エリア分布、アプリ別DL数・起動数比較 |
