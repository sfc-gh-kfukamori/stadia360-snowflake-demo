...
│         │                      │                                  │
│  ┌──────────────┐   ┌──────────────────────────────┐             │
│  │STADIA360     │   │  Cortex Search Service        │             │
│  │_TERMS (14行) │──▶│  STADIA360_TERMS_SEARCH       │             │
│  │(利用規約)    │   │  (semantic search)             │             │
│  └──────────────┘   └──────────┬───────────────────┘             │
│         │                      │                                  │
│         │          ┌───────────┴───────────┐                      │
│         │          │                       │                      │
...
│  │ │ (Altair)│ │(COMPLETE)││   │ + cortex_search      │          │
│  │ │PDF/Excel│ │          ││   │ + data_to_chart      │          │
│  │ │ Export  │ │          ││   │ + web_search         │          │
│  │ └─────────┘ └──────────┘│   └─────────────────────┘          │
│  └─────────────────────────┘                                      │
└──────────────────────────────────────────────────────────────────┘
...
| ファイル | 説明 |
|---|---|
| `stadia360_setup.sql` | テーブル作成、ダミーデータ投入、Semantic View 定義、Cortex Search Service 作成の SQL |
| `stadia360_dashboard_sis.py` | Streamlit in Snowflake 用ダッシュボードアプリ（PDF/Excelエクスポート機能付き） |
| `stadia360_agent_spec.json` | Cortex Agent の仕様定義（参考用。cortex_analyst + cortex_search + data_to_chart + web_search） |
| `environment.yml` | SiS 用パッケージ定義（fpdf2, openpyxl） |
| `STADIA360_Terms_of_Service.pdf` | STADIA360 利用規約 PDF（14セクション、17ページ） |
...
| **STADIA360_TERMS** | 14 | 利用規約。セクション別の規約テキスト（Cortex Search 用） |
...
- 利用規約テーブル `STADIA360_TERMS`（14セクション）の作成とデータ投入
- Cortex Search Service `STADIA360_TERMS_SEARCH` の作成
...
# environment.yml のアップロード（fpdf2, openpyxl パッケージ定義）
snow stage copy environment.yml \
  @KFUKAMORI_GEN_DB.STADIA360.STADIA360_STAGE/ \
  --overwrite -c <connection_name>
...
CREATE OR REPLACE AGENT KFUKAMORI_GEN_DB.STADIA360.STADIA360_AGENT
  ...
  instructions:
    orchestration: "あなたは電通のSTADIA360統合マーケティング基盤のデータ分析スペシャリストです。数値データの分析にはstadia360_analyticsツールを使い、利用規約・ポリシー・SLA・料金に関する質問にはstadia360_terms_searchツールを使ってください。回答は日本語で行ってください。"
  ...
  tools:
    ...
    - tool_spec:
        type: "cortex_search"
        name: "stadia360_terms_search"
        description: "STADIA360の利用規約・料金プラン・SLA・プライバシーポリシー等を検索するツール"
    - tool_spec:
        type: "data_to_chart"
        name: "data_to_chart"
  ...
  tool_resources:
    ...
    stadia360_terms_search:
      name: "KFUKAMORI_GEN_DB.STADIA360.STADIA360_TERMS_SEARCH"
      max_results: 5
...
| **レポートエクスポート** | PDF / Excel 形式でのレポート出力（ステージ経由のPresigned URLダウンロード） |
...
### 利用規約検索の質問例（Cortex Search）

- 「STADIA360の料金プランを教えてください」
- 「SLAの稼働率保証は何%ですか？」
- 「データの保存期間はどのくらいですか？」
- 「解約時のデータ取り扱いはどうなりますか？」
...
1. ユーザーが日本語で質問を入力
2. Cortex Agent がツールを自動選択:
   - **データ分析**: Semantic View (`STADIA360_ANALYTICS`) を参照して質問を SQL に変換・実行
   - **利用規約検索**: Cortex Search Service (`STADIA360_TERMS_SEARCH`) で利用規約テキストをセマンティック検索
3. 結果を日本語で回答
...
### Cortex Search Service の構成

| 項目 | 値 |
|---|---|
| サービス名 | `STADIA360_TERMS_SEARCH` |
| 検索カラム | `CONTENT`（利用規約本文） |
| 属性カラム | `SECTION_TITLE`, `CATEGORY` |
| 埋め込みモデル | `snowflake-arctic-embed-m-v1.5` |
| ソーステーブル | `STADIA360_TERMS`（14セクション） |
| リフレッシュモード | INCREMENTAL（TARGET_LAG = 1 day） |
...
### SiS での PDF/Excel エクスポート

SiS 環境では `st.download_button` がバイナリデータに対して S3 `SignatureDoesNotMatch` エラーを起こす場合があります。代替として以下のステージ経由のアプローチを使用しています:

1. `fpdf2` / `openpyxl` でバイナリデータを生成
2. `session.file.put_stream()` でステージにアップロード
3. `GET_PRESIGNED_URL()` で一時ダウンロード URL を生成
4. `st.markdown("[ファイル名](url)")` でダウンロードリンクを表示

```python
stream = io.BytesIO(data_bytes)
session.file.put_stream(stream, stage_path, auto_compress=False, overwrite=True)
url = session.sql(f"SELECT GET_PRESIGNED_URL(@STAGE, 'path', 3600) AS URL").collect()[0]["URL"]
st.markdown(f"[{file_name}]({url})")
```

> **注意**: `environment.yml` に `fpdf2` と `openpyxl` の依存を定義し、ステージのルートにアップロードする必要があります。
